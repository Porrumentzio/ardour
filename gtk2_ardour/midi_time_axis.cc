...
{
	cerr << "MidiTimeAxisView::use_midnam_info\n";
...
MidiTimeAxisView::model_changed (const std::string& m)
{
	...
	cout << "MidiTimeAxisView::model_changed model '" << model << "' mode '" << current_mode << "'\n";
...
MidiTimeAxisView::custom_device_mode_changed(const std::string& mode)
{
	...
	cout << "MidiTimeAxisView::model_changed model: " << model << " mode: " << mode << "\n";
	/* inform the backend, route owned instrument info */
	...
	if (apply_to_selection) {
		...
		if (midi_track()) {
			...
			if (/*gui_property (X_("midnam-model-name")) != DEFAULT_MIDNAM_MODEL && */ device_names && !device_names->controls().empty()) {
				...
				const std::string device_mode       = _route->instrument_info().mode (); // XXX
				const uint16_t    selected_channels = midi_track()->get_playback_channel_mask();
				...
				for (uint32_t chn = 0; chn < 16; ++chn) {
					...
					//TODO -> InstrumentInfo API
					boost::shared_ptr<ChannelNameSet> chan_names = device_names->channel_name_set_by_channel (device_mode, chn);
					if (!chan_names) {
						continue;
					}

					boost::shared_ptr<ControlNameList> control_names = device_names->control_name_list(chan_names->control_list_name());
					if (!control_names) {
						continue;
					}
